...
public class ScrambleDecApp {
    public static void main(String[] args) throws IOException {
        List<Integer> permutation = Arrays.asList(2, 5, 3, 4, 0, 1, 0, 4, 3, 2, 1);
        int groupSize = 6;
        Key key = new Key(groupSize, permutation);

        String encryptedText = readLine("src\\main\\resources\\scrambleDecSource");

        Path path = Paths.get("src\\main\\resources\\dest");
        ScrambleEncryption.decryptAndSaveToFile(encryptedText, key, path);
    }
}
